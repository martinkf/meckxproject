...
--we need to check a fix 60 slots? or we want to check only the available steps ._ .
totalEDLElements = 60;
EDLEy = 46;
EDLEx = 14;
...
local function ListUpdaterInfinite(self, pn, index)
	-- we are still using 29 steps at max, 30 and 31 are placeholders for 'negative' indexes
	-- he visto songs con mas de 50 charts, muy insano a mi parecer XD
    ...
    if topScreen:GetName() == "ScreenSelectMusic" and (topScreen:GetSelectionState() == "SelectingSteps" or topScreen:GetSelectionState() == "ConfirmSteps") and GAMESTATE:IsHumanPlayer(pn) and not GAMESTATE:GetMusicTrainChannel() and not GAMESTATE:GetProgressiveChannel() then
			
            local step, custom = GetCurrentStepfromList(index);
			...
			local displayRange = 3
			--limite (pares + offset 0)
			local visibleElements = (displayRange * 2) + 1
			local positionOffset =  index - activeIndex;
			...
			local isVisible = math.abs(positionOffset) <= displayRange-1
		...
		--	local newx = positionOffset * (pn == PLAYER_1 and 1 or -1) * EDLEx;
			local df = math.max(math.abs(positionOffset),0)
					
			if isVisible then	
				if newy ~= self:GetY() then
					self:stoptweening():linear(0.025):y(newy):x( newx ):z(math.abs(positionOffset)*-24);
				end;
				self:diffusealpha(1-(df*.46));
				...
				self:y(newy):x( newx ):z(math.abs(positionOffset)*-24);
				self:diffusealpha(0);
				children.scoreNumberBack:diffusealpha(0);
				children.scoreNumberFront:diffusealpha(0);
				children.gradescore:diffusealpha(0);				
			end
    end
	...
	for i = 1, totalEDLElements, 1 do
		t[#t + 1] = Def.ActorFrame {
			...
			SongChosenMessageCommand=function(self)
				self:stoptweening():
					diffusealpha(0):decelerate(0.2):diffusealpha(1);
					inSelectMode = true;
			end;
			SongUnchosenMessageCommand=function(self)
				self:stoptweening():
					diffusealpha(1):accelerate(0.2):diffusealpha(0);
					inSelectMode = false;
			end;
			StepDetailView(pn)..{
				InitCommand = function(self)
					...
					self:SetUpdateFunction(function(self)
							--why we need to call this thing like a chillion times a second to mantain the lv list? wtf is goin on.
							--arka
								local actualSongData = GetSteps();
								local activeForUpdate = false;
								if actualSongData ~= nil then 
									if i <= #actualSongData then -- we dont need to call this 60 fucking times when we only need to update 2 or 3 or whatever num of steps available xD
							...
							if inSelectMode and activeForUpdate then
								ListUpdaterInfinite(self, pn, i)
							end;
					end)
				end;
				OnCommand=function(self)
