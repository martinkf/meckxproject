...
local t = Def.ActorFrame{
	...
	LoadActor(THEME:GetPathS("","INFINITY _titleloop")) .. {
		OnCommand=cmd(queuecommand,"Sound");
		SoundCommand=function(self)
			if PREFSMAN:GetPreference('MusicTitle') then
				if (GAMESTATE:GetCoins() > 0 or GAMESTATE:GetCoinMode() == "CoinMode_Free" or GAMESTATE:GetCoinMode() == "CoinMode_Home") and GAMESTATE:GetGameMode() == "Infinity" and not BPlaying then
					self:play();	
					BPlaying=true;
		...
		PrevModMessageCommand=function(self,params)
			if GAMESTATE:GetGameMode() == "Infinity" and not BPlaying then
				self:play();
				BPlaying=true;
			else
				self:stop();
	...
	-- LoadActor(THEME:GetPathG("","TITLE/INFINITY.MPG"))..
	-- {
	...
	------------------PLAYER_1-------------------
	LoadActor(THEME:GetPathG("","TITLE/INFINITYPressCenterStep"))..
	{
		OnCommand=cmd(zoom,1.2;xy,200,SCREEN_CENTER_Y + 160);
	};	
	------------------PLAYER_2-------------------
	LoadActor(THEME:GetPathG("","TITLE/INFINITYPressCenterStep"))..
	{
		OnCommand=cmd(zoom,1.2;xy,SCREEN_WIDTH - 200,SCREEN_CENTER_Y + 160);
	};	
	
	
	LoadActor(THEME:GetPathG("","TITLE/INFINITYUSB")) .. 
	{
		OnCommand=function(self)
			self:zoom(1.5):xy(SCREEN_CENTER_X,SCREEN_CENTER_Y + 358);
		end;
	};
};
